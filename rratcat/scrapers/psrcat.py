...
def psrCatCheckUpdates(cat: dict) -> dict:
	...
	progressIter = tqdm.tqdm(enumerate(cat), total = len(cat))
	for idx, entry in progressIter:
		name = entry['NAME']
		progressIter.set_description(name)
		progressIter.refresh()
		...
		for key in baseKeys:
			...
			if key in ['RAJ', 'DECJ']:
				...
				output[idx] = setPosition(output[idx], rajStr, decjStr, "PSRCAT")
				continue
